...
pub fn transaction(props: &Props) -> Html {
   ...
   let payload = polkadot::tx()
        .project_tips()
        .apply_jurors_project_tips(project_id, stake);
...
pub fn conditional_transaction_extension(props: &Props) -> Html {
    ...
    if account_address.is_none() || account_source.is_none() {
        ...
        html! {
            <>
            <TransactionExtension project_id={project_id} stake={stake} account_address={account_address} account_source={account_source} />
            </>
        }
    } else {
        html! {
...
pub fn conditional_transaction(props: &Props) -> Html {
    ...
    if let SignInMethod::ExtensionSignIn = sign_in_method {
        html! {
            <ConditionalTransactionExtension  project_id={project_id} stake={stake} />
        }
    } else {
        if store.phase_exists_in_state == false {
            ...
            html! {
             <>
             <Transaction  project_id={project_id} stake={stake}/>
             </>
            }
        }
    }
