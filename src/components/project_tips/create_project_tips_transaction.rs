...
pub fn transaction(props: &Props) -> Html {
    ...
    let create_project_tips_tx = polkadot::tx()
        .project_tips()
        .create_project(department_id, tipping_name, funding_needed);

    let hookdata = use_sign_tx(create_project_tips_tx);
...
pub fn transaction_extension(props: &ExtensionProps) -> Html {
    ...
    let hookdata = use_sign_tx_extension(create_project_tips_tx, account_address, account_source);
