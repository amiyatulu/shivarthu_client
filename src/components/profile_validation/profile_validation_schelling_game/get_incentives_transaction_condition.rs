...
pub fn transaction(props: &Props) -> Html {
    ...
    let get_incentives_tx = polkadot::tx()
        .profile_validation()
        .get_incentives(account_id32);

    let hookdata = use_sign_tx(get_incentives_tx);
...
pub fn transaction_extension(props: &ExtensionProps) -> Html {
    ...
    let hookdata = use_sign_tx_extension(get_incentives_tx, account_address, account_source);
    html! {
        <>
