...
pub fn conditional_transaction_extension(props: &Props) -> Html {
    ...
    if account_address.is_none() || account_source.is_none() {
        ...
        html! {
            <>
            <TransactionExtension user_to_calculate={user_to_calculate} choice={choice} salt_string={salt_string} account_address={account_address} account_source={account_source} />
            </>
        }
    } else {
        html! {
...
pub fn conditional_transaction(props: &Props) -> Html {
    ...
    if let SignInMethod::ExtensionSignIn = sign_in_method {
        html! {
            <ConditionalTransactionExtension  user_to_calculate={user_to_calculate} choice={choice}  salt_string={salt_string} />
        }
    } else {
        if store.phase_exists_in_state == false {
            ...
            html! {
             <>
             <Transaction  user_to_calculate={user_to_calculate} choice={choice}  salt_string={salt_string} />
             </>
            }
        }
    }
