...
pub fn transaction(props: &Props) -> Html {
    ...
    let draw_juror_tx = polkadot::tx()
        .positive_externality_validation()
        .draw_jurors_positive_externality(account_id32, iterations);
...
pub fn transaction_extension(props: &ExtensionProps) -> Html {
    ...
    let hookdata = use_sign_tx_extension(draw_juror_tx, account_address, account_source);
    html! {
        <>
...
pub fn conditional_transaction_extension(props: &Props) -> Html {
    ...
    if account_address.is_none() || account_source.is_none() {
        ...
        html! {
            <>
            <TransactionExtension user_to_calculate={user_to_calculate} iterations={iterations} account_address={account_address} account_source={account_source} />
            </>
        }
    } else {
        html! {
...
pub fn conditional_transaction(props: &Props) -> Html {
    ...
    if let SignInMethod::ExtensionSignIn = sign_in_method {
        html! {
            <ConditionalTransactionExtension  user_to_calculate={user_to_calculate} iterations={iterations} />
        }
    } else {
        if store.phase_exists_in_state == false {
            ...
            html! {
             <>
             <Transaction  user_to_calculate={user_to_calculate} iterations={iterations}/>
             </>
            }
        }
    }
