...
use yew::prelude::*;
use std::ops::Deref;
use crate::components::positive_externality_validation::reveal_vote_positive_externality_transaction_condition::ConditionalTransactionModal;
...
pub fn reveal_vote(props: &Props) -> Html {
    ...
    let choice_state: UseStateHandle<Option<i64>> = use_state(|| None);
    ...
    if *submit_done_clone == false {
        ...
        html! {
            <>
            <ConditionalTransactionModal salt_string={salt_string} user_to_calculate={user_to_calculate} choice={choice}/>
            </>
        }
    }
}
